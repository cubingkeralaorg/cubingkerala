...
import { DELETE } from '@/app/api/delete-member/route';
...
function createMockRequest(options: {
  ...
  const request = new NextRequest('http://localhost:3000/api/delete-member', {
    method: 'DELETE',
...
describe('DELETE /api/delete-member', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  describe('Request Body Validation', () => {
    ...
    it('should return 400 when wcaid is invalid format', async () => {
      const request = createMockRequest({
        body: { wcaid: '' },
        cookies: {
          userInfo: JSON.stringify(mockUserInfo),
  ...
  describe('Successful Member Deletion', () => {
    it('should delete member and return success', async () => {
      mockDb.members.delete.mockResolvedValue({
        wcaid: '2023TEST01',
        name: 'Test User',
      });
      ...
      expect(mockDb.members.delete).toHaveBeenCalledWith({
  ...
  describe('Error Handling', () => {
    it('should return 404 when member not found (P2025)', async () => {
      ...
      mockDb.members.delete.mockRejectedValue(prismaError);
    ...
    it('should return 500 when database throws generic error', async () => {
      mockDb.members.delete.mockRejectedValue(new Error('Database error'));
